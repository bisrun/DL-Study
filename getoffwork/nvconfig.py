...
class nvconfig :
    ...
    def load_file(self):
        try:
            ...
            self._loginId = config['COMMON']['LOGINID']
            self._loginPwd =  config['COMMON']['LOGINPWD']
            self._initUrl = config['COMMON']['INITURL']
            self._DriverPath =os.path.join(self._config_dir_path, config['COMMON']['DRIVERNAME'])
